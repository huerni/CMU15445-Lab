...
void NestedLoopJoinExecutor::Init() { 
  ...
  for(auto& left_tuple : left_tuples) {
    ...
    for(auto& right_tuple : right_tuples) {
      Value isMatch = plan_->Predicate().EvaluateJoin(&left_tuple, left_executor_->GetOutputSchema(), &right_tuple, right_executor_->GetOutputSchema());
      if(isMatch.ToString() == "true") {
    ...
    if(plan_->GetJoinType() == JoinType::LEFT && matchFlag == false){
        std::vector<Value> values{};
        for(size_t i = 0; i<left_executor_->GetOutputSchema().GetColumnCount(); ++i) {
          values.push_back(left_tuple.GetValue(&left_executor_->GetOutputSchema(), i));
        }
